...
class PantallaPrivado extends StatelessWidget {
  const PantallaPrivado({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('🔒 Privadas'),
        backgroundColor: AppCSS.verdePrimario,
        centerTitle: true,
        actions: [IconButton(icon: const Icon(Icons.add_circle_outline, color: AppCSS.blanco), onPressed: () => _mostrar(context))],
      ),
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final citas = snapshot.data!.docs.map((d) => CitaRegistro.fromFirestore(d)).where((c) => !c.publico && c.email == userEmail).toList();
          if (citas.isEmpty) return SinDatos(mensaje: 'No tienes citas privadas', icono: Icons.lock, textoBoton: 'Agregar', accionBoton: () => _mostrar(context));
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Icon(Icons.lock, size: 14, color: AppCSS.gris),
                ...
                IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red, size: 20), padding: EdgeInsets.zero, constraints: const BoxConstraints(), onPressed: () => _eliminar(context, c)),
              ]),
